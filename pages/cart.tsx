		...
		<div>
			<div>
				...
				<div className="container mx-auto my-4 max-w-5xl border border-gray-400 rounded-lg p-4 flex flex-col gap-8">
					{no_of_item_in_cart > 0 ? cartItem.map((item: IProduct, idx: Key) => {
						return (
					...
					{no_of_item_in_cart > 0 && <button className="border border-gray-300 rounded-xl font-medium text-gray-600 p-4 hover:bg-gray-200" onClick={(_) => setCartItem([])}>Checkout</button>}
				</div>
			</div>
		</div >
	);
